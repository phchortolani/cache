// Modified Date: 2009-09-23 20:00:34
// Modified Language: 767253605
...
function GridClick(pEvent,pKey) {
	...
	var idField=event.srcElement.id.split('_')[1];
	CallBack("GridClick^COMViewFilter",pKey,Back,0,idField);
}
function SelectAll() {
...
function AddView(pValue,pText,pSelected,pDistribute) {
	...
	el.style.fontWeight='bold';
	if (pDistribute==0) el.style.color='darkblue'; //16443
	el.innerHTML=' '+pText;
	el.value=pValue;
	filt.appendChild(el);
}
...
function ControlChanged() {
    ...
    if (event.keyCode==13) {	//SR14763
		...
		if (el.checkbox=='tristate') {  //BR014967
			value= el.parentNode.checked;  //BR014967		
		} else {
...
function BuildCOMViewStructure(pblnInForm,pblnIsSuperUser,pblnUseKeyStroke) {
 ...
 obj2=cvCreateButton(obj,'hback.gif','Back',cvBack,'cvBackButton');
