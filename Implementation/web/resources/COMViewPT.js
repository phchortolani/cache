// Modified Date: 2006-08-28 09:16:36
// Modified Language: 1572159339
...
function Add(GridBody,Row,Cells,Key,Align,Short,Group) {
	...
	for (var i=0;i<arrCells.length;i++) {
		...
		objCell.title='Clique no botão direito para ver opções disponíveis';  //Right click cell to view available options 
	}
	if (Group!=null) {
...
function SetStatus(pstrStatus) {		//SR14763 - set the search status
...
function RowEvent() {
	...
	if (event.srcElement.type!='select-one') {
		if (bdy.rows.length>0) {
			...
			if (KeyCode == 13) {  //Enter
				//SubmitQuery();				//SR14763 - not consistent
...
function NewView() {
	ViewName=window.prompt('Registrar Descrição '+":",'Novo Favorito ');  //Enter Description  //New Favourite
	if (ViewName!=null) {
...
function DeleteView() {
	...
	var result=VBConfirm('Tem certeza de que quer excluir o Favorito: '+ViewDescription+"?",33,'Excluir Confirmação ');  //;Are you sure you want to delete favourite:  //Delete Confirm
	if (result==1) {
...
function ControlChanged() {
    ...
    var idFilter = el.id;
    //SR14763
    var blnSubmit = (event.keyCode==13) || (document.getElementById('blnKeyStroke').value);
    if (blnSubmit && (idFilter.search(/(value|comp|group|Submit)/)!=-1)) {
        SetStatus('Com00256');
    }
	...
	if (el.type=='checkbox') {
	    ...
	    if (value!=el.initialValue) {
		    ...
		    if (event.keyCode==13) { //SR14613
		    	CallBack('ControlChanged^COMViewFilterControl',idFilter,(value==null)?'':value,true);
		    }
	    }
	}
    if (Status==false) {
...
//</script>
	...
	function BuildCOMViewStructure() {
	 ...
	 obj4.innerHTML=4;
	 obj4.title='Escolha de Campo'  //Field Chooser
	 ...
	 obj2.innerHTML='Favoritos';  //Favourites
	 ...
	 obj.id='buttonpanel';
	   cvCreateButton(obj,'new.gif','Novo ',NewView,'newFavourite'); // SR14249
	   cvCreateButton(obj,'open.gif','Abrir',EditView,'openFavourite'); // SR14249
	   cvCreateButton(obj,'save.gif','Salvar ',SaveView,'saveFavourite'); // SR14249
	   cvCreateButton(obj,'del.gif','Excluir ',DeleteView,'deleteFavourite'); // SR14249
	   obj2=document.createElement('img');obj.appendChild(obj2);
	   obj2.src=FilePath+'delimiter.gif';
	   obj2=cvCreateButton(obj,'print.gif','Imprimir ',PrintSelection,'printResultset'); // SR14249
	   obj2.attachEvent('oncontextmenu',PrintOptions);
	   obj2=cvCreateButton(obj,'copy.gif','Copiar ',CopySelection,'copyResultset'); // SR14249
	   obj2=cvCreateButton(obj,'selectall.gif','Selecionar ',SelectAll,'selectallResultset'); // SR14249
...
obj2=cvCreateButton(obj,'grid.gif','Exportar para CSV',ExportToCSV,'ExportToCSV');
	 cvCreateButton(obj,'ok.gif','Procurar ',SubmitQuery,'SubmitQuery'); // SR14613
	   obj=document.createElement('span'); objDiv.appendChild(obj);
	   obj.id = 'blnKeyStroke'; 	   obj.value = false; 	   obj=document.createElement('br');objDiv1.appendChild(obj);
	  obj=document.createElement('fieldset');objDiv1.appendChild(obj);
	  obj.className='fieldset-ctrl';
	   obj2=document.createElement('legend');obj.appendChild(obj2);
	   obj2.className='legend';
	   obj2.innerHTML='Seleção de Campo ';    // Field Selection
