...
//    ['HorizontalRule','Createlink','InsertImage','InsertTable','htmlmode','separator'],
    ['HorizontalRule','InsertImage','htmlmode','separator'],
      ['popupeditor']
  ];
...
//this.stylesheet = "http://172.16.0.11/BIN/htmlarea/editor.css"; // full URL to stylesheet

this.stylesheet = config.css[0]
...
this.btnList = {
    ...
    "bold":           ['Bold',config.tooltip[1],               'editor_action(this.id)',  'ed_format_bold.gif'],
    "italic":         ['Italic',config.tooltip[2],             'editor_action(this.id)',  'ed_format_italic.gif'],
    "underline":      ['Underline',config.tooltip[3],          'editor_action(this.id)',  'ed_format_underline.gif'],
    "strikethrough":  ['StrikeThrough',config.tooltip[4],      'editor_action(this.id)',  'ed_format_strike.gif'],
    "subscript":      ['SubScript',config.tooltip[5],          'editor_action(this.id)',  'ed_format_sub.gif'],
    "superscript":    ['SuperScript',config.tooltip[6],        'editor_action(this.id)',  'ed_format_sup.gif'],
    "justifyleft":    ['JustifyLeft',config.tooltip[7],       'editor_action(this.id)',  'ed_align_left.gif'],
    "justifycenter":  ['JustifyCenter',config.tooltip[8],     'editor_action(this.id)',  'ed_align_center.gif'],
    "justifyright":   ['JustifyRight',config.tooltip[9],      'editor_action(this.id)',  'ed_align_right.gif'],
    "orderedlist":    ['InsertOrderedList',config.tooltip[10],       'editor_action(this.id)',  'ed_list_num.gif'],
    "unorderedlist":  ['InsertUnorderedList',config.tooltip[11],      'editor_action(this.id)',  'ed_list_bullet.gif'],
    "outdent":        ['Outdent',config.tooltip[12],    'editor_action(this.id)',  'ed_indent_less.gif'],
    "indent":         ['Indent', config.tooltip[13],    'editor_action(this.id)',  'ed_indent_more.gif'],
    "forecolor":      ['ForeColor',config.tooltip[14],         'editor_action(this.id)',  'ed_color_fg.gif'],
    "backcolor":      ['BackColor',config.tooltip[15],   'editor_action(this.id)',  'ed_color_bg.gif'],
    "horizontalrule": ['InsertHorizontalRule',config.tooltip[16],    'editor_action(this.id)',  'ed_hr.gif'],
    "createlink":     ['CreateLink',config.tooltip[17],    'editor_action(this.id)',  'ed_link.gif'],
    "insertimage":    ['InsertImage',config.tooltip[18],       'editor_action(this.id)',  'ed_image.gif'],
    "inserttable":    ['InsertTable',config.tooltip[19],       'editor_action(this.id)',  'insert_table.gif'],
    "htmlmode":       ['HtmlMode',config.tooltip[20],   'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
    "popupeditor":    ['popupeditor',config.tooltip[21],     'editor_action(this.id)',  'fullscreen_maximize.gif'],
    "about":          ['about',config.tooltip[22],  'editor_about(\''+objname+'\')',  'ed_about.gif'],

    // Add custom buttons here:
   // "custom1":           ['custom1',         'Save',  'editor_action(this.id)',  'save.gif'],
    "custom1":           ['custom1',config.tooltip[23],  'editor_action(this.id)',  'save.gif'],
   "custom2":           ['custom2',config.tooltip[24],  'editor_action(this.id)',  'help.gif'],
    "custom3":           ['custom3',config.tooltip[25],  'editor_action(this.id)',  'listg.gif'],
   // end: custom buttons
...
function editor_generate(objname,userConfig) {
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      toolbar += '<button title="' +btnTitle+ '" id="_' +objname+ '_' +btnCmdID+ '" class="btn" onClick="' +btnOnClick+ '" onmouseover="if(this.className==\'btn\'){this.className=\'btnOver\'}" onmouseout="if(this.className==\'btnOver\'){this.className=\'btn\'}" unselectable="on"><img src="' +config.imgURL + btnImage+ '" border=0 width=18 unselectable="on"></button>';
