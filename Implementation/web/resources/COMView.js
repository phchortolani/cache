...
function SAVENOW() {
	if (document.WWW.YOPEN.value=='SAVESEAR') {
		ShowSearch();
 		...
 		window.focus();
 		setTimeout('document.WWW.submit()',300);
	} 
}
...
function Add(GridBody,Row,Cells,Key,Align,Short) {
	var Data,objCell
	...
	for (var i=0;i<arrCells.length;i++) {
		...
		objCell.title='Right click cell to be view available options';  //Right click cell to be view available options
	}
...
function CellContextMenu(pintFlag) {
	var result=true,
	    idField=hdr.cells[event.srcElement.id.split('_')[1]].Field;
	var Key=eval(event.srcElement.parentNode.id+'.Key');
	if (DragTimeOut!=null) {
		window.clearTimeout(DragTimeOut);
		DragTimeOut=null;
	}
	CallBack("Show^COMViewColumnMenu",Key,idField,pintFlag,event.srcElement.id);
...
function RowEvent() {
	...
	if (event.srcElement.type!='select-one') {
		if (bdy.rows.length>0) {
			...
			} else if (KeyCode==120) {  //Search show/hide
...
function GridClick(pEvent,pKey) {
	if (pKey==null) {
		pKey=event.srcElement.parentNode.Key;
	}
	var Back='';
...
function RowMouseOut() {
	var objRow=event.srcElement.parentNode;
	if (objRow.Row!=CurrentRow) {
...
function AddChooseField(ChooserBody,pidField,pstrDesc,pstrRelation) {
...
function AddCol(GridHead,Text,Width,SortDirection,ColNumber) {
	...
	objHdr.className='head';
 	...
 	if (arguments.length==6) { // SR10965
	 	objHdr.title=arguments[5];
 	} else {
		objHdr.title=Text;
 	}
...
function NewView() {
	ViewName=window.prompt("Enter Description:","New Favorite");
	if (ViewName!=null) {
...
function DeleteView() {
	...
	var result=VBConfirm("Are you sure you want to delete favourite: "+ViewDescription+"?",33,"Delete Confirm");
	if (result==1) {
...
function ControlChanged() {
    ...
    if (el.type=='checkbox') {
	    ...
	    Status=true;
	    CallBack('ControlChanged^COMViewFilterControl',el.id,value);
    } else {
	    ...
	    if (value!=el.initialValue) {
    		...
    		if (event.type=="keyup"){
		    	...
		    	var ExecuteString="CallBack('ControlChanged^COMViewFilterControl','"+el.id+"','"+value+"')"
...
function CreateControl(pidControl,pidField) {
   	var objRow = document.createElement('tr');
   	...
   	objCell.style.fontSize=12;
	objCell.attachEvent('ondblclick',RemoveControl)
...
//<script type='text/javascript'><!--
	function BuildCOMViewStructure() {
	 ...
	 objDiv=document.createElement('div');WWW.insertAdjacentElement('BeforeBegin',objDiv);
	 objDiv.style.display='none';
	 objDiv.style.backgroundColor=NetBorderColor;
	 objDiv.className='obj-div'; 
	 objDiv.id='objDIV';
	 objDiv.attachEvent('onkeydown',RowEvent);
	 objDiv.attachEvent('onclick',SetFocus);
	 ...
	 obj.style.height='100%';
	  ...
	  obj.className='fieldset-save';
	   ...
	   obj2.src=FilePath+'new.gif';
	   obj2.className='coolButton';
	   obj2.attachEvent('onclick',NewView);
	   obj2=document.createElement('img');obj.appendChild(obj2);
	   obj2.src=FilePath+'open.gif';
	   obj2.className='coolButton';
	   obj2.attachEvent('onclick',EditView);
	   obj2=document.createElement('img');obj.appendChild(obj2);
	   obj2.src=FilePath+'save.gif';
	   obj2.className='coolButton';
	   obj2.attachEvent('onclick',SaveView);
	   obj2=document.createElement('img');obj.appendChild(obj2);
	   obj2.src=FilePath+'del.gif';
	   obj2.className='coolButton';
	   obj2.attachEvent('onclick',DeleteView);
	   obj2=document.createElement('img');obj.appendChild(obj2);
	   obj2.src=FilePath+'delimiter.gif';
	   obj2=document.createElement('img');obj.appendChild(obj2);
	   obj2.src=FilePath+'print.gif';
	   obj2.className='coolButton';
	   obj2.attachEvent('onclick',PrintSelection);
	   obj2=document.createElement('img');obj.appendChild(obj2);
	   obj2.src=FilePath+'copy.gif';
	   obj2.className='coolButton';
	   obj2.attachEvent('onclick',CopySelection);
	  obj=document.createElement('br');objDiv1.appendChild(obj);
	  obj=document.createElement('fieldset');objDiv1.appendChild(obj);
	  obj.className='fieldset-ctrl';
	   obj2=document.createElement('legend');obj.appendChild(obj2);
	   obj2.className='legend';obj2.innerHTML='Field Selection';obj.appendChild(obj2);
	   obj2=document.createElement('div');obj.appendChild(obj2);
	   obj2.className='legend';
	   obj2.id='hdrctrl';
	   ...
	   obj2.className='ctrltable';
		...
		obj3.id='ctrl';
		obj3.attachEvent('ondragover',ChooseFieldDrag);
		obj3.attachEvent('ondragenter',ChooseFieldDrag);
		obj3.attachEvent('ondrop',ChooseFieldEnd);
	  obj=document.createElement('div');objDiv1.appendChild(obj);
	  obj.className='hdr-div';
	  obj.id='hdrDiv';
      obj.attachEvent('ondragover',ChooseFieldDrag);
	  obj.attachEvent('ondragenter',ChooseFieldDrag);
	  obj.attachEvent('ondrop',ChooseFieldEnd);
